...
def request_json(endpoint, **kwds):
    ...
    print(f"Couldn't access {endpoint}: {error}")
    return None
...
class SciCrunch(object):
    def __init__(self):
        ...
        if self.__scigraph_key is None:
            print('Undefined SCICRUNCH_API_KEY: SciCrunch knowledge will not be looked up')
        self.__label_cache = {}
    ...
    def get_knowledge(self, entity):
        ...
        if len(knowledge) == 0 and entity not in self.__unknown_entities:
            print(f'Unknown anatomical entity: {entity}')
            self.__unknown_entities.append(entity)
        return knowledge

    def label(self, entity):
    #=======================
        if entity in self.__label_cache:
            return self.__label_cache[entity]
        knowledge = self.get_knowledge(entity)
        label = knowledge.get('label')
        self.__label_cache[entity] = label
        return label
